...
import requests
from cumulusci.cli.utils import (
...
def run_crm_analytics_checks(org_name):
    ...
    if org_name:
        run_cci_task(
            "analytics_manager", org_name, mode="d", generate_metadata_desc=True
        )
...
def cumulusci_update_check():
    ...
    try:
        ...
        log.error(
            "There was an issue retrieving the latest CumulusCI version. Skipping task"
        )
    ...
    if result:
        try:
            ...
            log.error(
                " -X Error executing command to update CumulusCI: %s", error_output
            )
...
def check_python_library_dependencies():
    ...
    run_cci_task(
        "command",
        org_name=None,
        command="pip install --upgrade pandas pandasql robotframework-browser",
    )
...
def check_and_update_nodejs():
    ...
    try:
        ...
        latest_lts_version_cmd = subprocess.run(
            ["npm", "view", "node", "dist-tags.lts"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            check=True,
            text=True,
        ...
        if installed_version < latest_lts_version:
            ...
            subprocess.run(
                ["npm", "install", "--global", f"node@{latest_lts_version}"], check=True
            )
        ...
        log.info(
            " -> Node.js is not installed or an error occurred while checking/updating."
        )
...
def get_template_info(template_id):
    ...
    try:
        ...
        check_template_url = (
            f"{base_url}/postspin/isknowntemplate/?templateid={template_id}"
        )
        ...
        if "result" in response_json and response_json["result"] == True:
            template_info_url = (
                f"{base_url}/postspin/latesttemplate/?templateid={template_id}"
            )
