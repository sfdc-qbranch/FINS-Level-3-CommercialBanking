...
class HealthChecker(BaseTask, ABC):
    ...
    def _init_options(self, kwargs):
        ...
        self.api_checker_include_code_files = (
            self.options["api_checker_include_code_files"]
            if "api_checker_include_code_files" in self.options
            else False
        )
        self.remove_standard_fields = (
            self.options["remove_standard_fields"]
            if "remove_standard_fields" in self.options
            else False
        )
        self.remove_empty_translations = (
            self.options["remove_empty_translations"]
            if "remove_empty_translations" in self.options
            else False
        )
        self.auto_generate_external_id_fields = (
            self.options["auto_generate_external_id_fields"]
            if "auto_generate_external_id_fields" in self.options
            else False
        )
        self.regenerate_permission_set = (
            self.options["regenerate_permission_set"]
            if "regenerate_permission_set" in self.options
            else False
        )
        ...
        if self.regenerate_permission_set:
            ...
            if project_name:
                ...
                permission_set_check = (
                    input(
                        f"Do you want to overwrite any existing file located at force-app/main/default/permissionsets/{permission_set_file_name}.permissionset-meta.xml: (y/N) "
                    )
                    or "n"
                )
                if permission_set_check and (
                    permission_set_check.lower() == "y"
                    or permission_set_check.lower() == "n"
                ):
                    upsert_mode = True if permission_set_check.lower() == "y" else False
                    create_permission_set_file(
                        name=permission_set_file_name,
                        label=f"{project_name} Permission Set",
                        permission_set_path=None,
                        run_as_upsert=upsert_mode,
                    )
                    self.logger.info(" -> Check Complete!")
                else:
                    self.logger.info(
                        "There was an unexpected response, expecting y or n, skipping Permission Set check and rebuild."
                    )
    ...
    def check_project_file_naming(self):
        ...
        if repo_url is not None:
            ...
            repo_qbrix_name = self.project_config.project__git__repo_url.rsplit("/", 1)[
                -1
            ]
        ...
        if project_name is None or package_name is None or repo_url is None:
            ...
            if (
                "xDO-Template" in project_name
                or "xDO-Template" in package_name
                or "xDO-Template" in repo_url
            ):
                file_name_error = True
                log.error(
            ...
            if repo_qbrix_name not in get_json_file_value("orgs/dev.json", "orgName"):
                log.debug(
                    "Naming Check: Updating OrgName in orgs/dev.json has not been updated. Updating now..."
                )
                update_json_file_value(
                    "orgs/dev.json", "orgName", f"{repo_qbrix_name} - Dev org"
                )
                self.logger.info("Naming Check: Updated orgs/dev.json")

            # Check that the dev_preview.json file has the correct qbrix name
            if repo_qbrix_name not in get_json_file_value(
                "orgs/dev_preview.json", "orgName"
            ):
                log.debug(
                ...
                update_json_file_value(
                    ...
                    "orgName",
                    f"{repo_qbrix_name} - Preview Dev org",
                )
                log.debug("Naming Check: Updated orgs/dev_preview.json")
